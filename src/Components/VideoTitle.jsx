...
const VideoTitle = ({title, desc}) => {
  return (
    <div className='sm:space-y-5 space-y-2 sm:ml-10 ml-3 absolute sm:top-[50%] top-[35%] text-white'>
        <h1 className='sm:text-5xl text-xl font-bold'>{title}</h1>
        <p className=' slider sm:text-xl text-sm sm:w-1/2 w-5/6 sm:h-20 h-14 overflow-y-auto font-semibold'>{desc}</p>
